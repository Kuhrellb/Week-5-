...
class Brand {
    ...
    addBrand(brand) { // This passes through an instance of our brand and makes sure that only a brand is being passed.
        if (brand instanceof Brand) {
            this.brands.push(brand);
        } else {
            throw new Error(`You can only add an instance of Shoe. Argument is not a shoe: ${brand}`);
        }
    }
    describe() {
        return `This brand has ${this.shoes.length} shoes.`;//returns how many shoes each brand has
    }
}
class Menu {
    constructor() {
    ...
    createBrand() {
        let brand = prompt(`Enter new Brand Name: `); 
        console.log(brand)
        //Prompts the user to enter a new brand name tehy would like to add
    ...
    }//Takes user input to delete the shoes of their selection by selecting the index of the shoe they want to delete
}
